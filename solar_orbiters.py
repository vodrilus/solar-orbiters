...
TIME_SCALE = 20 # Computational steps per frame (supposedly 10 millisec per frame)


class MovementSystem(sdl2.ext.Applicator):
    def __init__(self):
        super(MovementSystem, self).__init__()
        self.componenttypes = Position, Velocity, Acceleration, Mass, sdl2.ext.Sprite

    def process(self, world, componentsets):
        """Apply naive gravity algorithm (O(n**2)) and move sprites accordingly."""
        ...
        for i in xrange(TIME_SCALE):
            for position, velocity, acceleration, mass, sprite in comps:
                ...
                for pos2, vel2, acc2, mass2, sp2 in comps:
                    x2, y2 = pos2.x, pos2.y
                    if x1 == x2 and y1 == y2 or mass2.mass < 100000:
                    ...
                    # object. (Eigenvalue)
                    a = grav_constant * mass2.mass / ((x2 - x1)**2 +
                                                      (y2 - y1)**2)
                ...
                """
                #sprite.x = int(max(self.minx, sprite.x))
                #sprite.y = int(max(self.miny, sprite.y))
